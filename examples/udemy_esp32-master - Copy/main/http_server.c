...
static esp_err_t http_server_index_html_handler(httpd_req_t *req)
{
    ...
    for (int i = 0; (i < ap_count); i++) {
		...
		httpd_resp_send_chunk(req, (const char *)ap_info[i].ssid, -1);
		httpd_resp_send_chunk(req, ">", -1);
		httpd_resp_send_chunk(req, (const char *)ap_info[i].ssid, -1);
		httpd_resp_send_chunk(req,"</option>", -1);
    }
	httpd_resp_send_chunk(req, (const char *)index1_html_start, index1_html_end - index1_html_start);
...
static esp_err_t http_server_config_handler(httpd_req_t *req)
{
	// ESP_LOGI(TAG, "favicon.ico requested");
...
static esp_err_t http_server_threshold_json_handler(httpd_req_t *req)
{
	...
	// Get SSID header
	len_ssid = httpd_req_get_hdr_value_len(req, "my--ssid") + 1;
	if (len_ssid > 1)
	{
		ssid_str = malloc(len_ssid);
		if (httpd_req_get_hdr_value_str(req, "my-connect-ssid", ssid_str, len_ssid) == ESP_OK)
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		httpd_uri_t config = {
				.uri = "/config.html",
				.method = HTTP_GET,
				.handler = http_server_config_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &config);
		...
		httpd_uri_t threshold_json = {
				.uri = "/threshold.json",
				.method = HTTP_POST,
				.handler = http_server_threshold_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &threshold_json);
